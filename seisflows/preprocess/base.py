...
class base(object):
	...
	tmp = 3.0*PAR.F0
        ...
        for tr in traces:
            ...
            tr.detrend('linear')
            tr.taper(0.5, type='hann')
            tr.filter('lowpass', freq=tmp)

        return traces

    def reweight_trace(self, traces):
        traces = signal.reweight_trace(traces,
        0.0)
        return traces


    def reweight_trace1(self, traces):
        traces = signal.reweight_trace1(traces,
